...
on $LastChangedDate: 2014-10-28 16:45:11 +0100 (Tue, 28 Oct 2014) $

--------------------------------------------------------------------------- */

#define DATE_STRING "$LastChangedDate: 2014-10-28 16:45:11 +0100 (Tue, 28 Oct 2014) $"
...
int getcoeffs_fits_expand_v5(char *, double **, int, int, int, int, int,
                             int *);
int getglobalpsfsize(char *, int *, int *, int *, int *, int *, int *, int *, int *,
...
/* Set the instrument here by defining the flag that is checked later
   in the code (currently KIDS, CFHT, SUPRIME or SINGLE) */
#define KIDS (1)
...
main(int argc, char *argv[])
{
    ...
    double *rx, *ry;
    double wcsneg[2], wcspos[2], rawpos[2], wcscentre[2];
    ...
    char *firstname, *badname, *ellname, *usedstarsname, *stampname;
    char **argkey, **argval;
    ...
    */
    ...
    getglobalpsfsize(coeffname, &CORRECT_DISTORTION, &pwidth, &pheight, &order,
                     ...
                     &nxchip, &nychip, &xchipsampling, &ychipsampling,
    ...
    if (USE_SWARP == 1)
      {
#ifdef KIDS
        strcpy(weight_suffix, "FCS.weight.fits");
	strcpy(delimiter,"F");
			  ...
			  (wcs_raw, apix, badpix, opix, dim, dpix,
			   ...
			   rawpos, wcspos, wcsneg, wcscentre, flagbad);
                    ...
                    if (ngoodpix > 0) {
                        ...
                        if (psfnorm > (double) (snratio * noisevalue)) {
                            ...
                            if (maxvalue <= satlimit
                                 ...
                                 */
                                psfmoments(bpix, pheight, pwidth, psfe,
                                           centroid, moments);
                                /*
                                 * FFT the PSF 
                                 */
					...
					if (xx*xx+yy*yy <= cchwidth*cchwidth)
					  {
                                 ...
                                 */
                                printf (" %f %f %f %f %f %f %f %f %f \n",rdegs[i],ddegs[i],objx[i],objy[i],shift[0],shift[1],centroid[0],centroid[1],cross);
				...
				/* measure moments of shifted PSF and data and output to file */
				    ...
				    fprintf(ellfile,"%d %d %f %f %f %f %f %f %f %f %f ", 
					    i+1, ichip+1, rdegs[i], ddegs[i], 
        ...
        for(iy = 0; iy < pheight; iy++) {
            for(ix = 0; ix < pwidth; ix++) {
                ...
                if (num_obj_used[image] > 0) {
                    diff[image][pixel] =
                        diff[image][pixel] / num_obj_used[image];
                } else {
                    diff[image][pixel] = 0.;
                }
            }
