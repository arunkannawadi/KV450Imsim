...
SVN Revision $Rev: 267 $ 
last changed by $Author: miller $
on $LastChangedDate: 2015-08-12 10:35:04 +0200 (Wed, 12 Aug 2015) $
...
#define DATE_STRING "$LastChangedDate: 2015-08-12 10:35:04 +0200 (Wed, 12 Aug 2015) $"
#define REV_STRING "$Rev: 267 $"
...
float Pcut = 0.02;
// set likelihood threshold at 0.02 in probability (i.e. enclosing 98% of a 2D gaussian)
float Lcut = 0.02;
...
/* read the input catalogue */
    ...
    for (i=0; i<nobj; i++) output_string[i] = (char*) calloc(190, sizeof(char));
    temp_string = (char*)calloc(190, sizeof(char));
		      ...
		      while (more==1 && nmore < nmorelimit && maxrfitvals > NOT_MEASURED)
			{
			  ...
			  for (ne=0; ne<nume; ne++)
			      ...
			      if (old_wanted[nt][ne]==0 && wanted[nt][ne]==1 )
				{
				  if (VERBOSE==1) 
				    {
				      printf(" thread %d object %d iteration %d added %d\n",nt,i+1,nmore,ne);
				    }
