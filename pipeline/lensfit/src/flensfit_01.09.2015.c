...
SVN Revision $Rev: 271 $ 
last changed by $Author: miller $
on $LastChangedDate: 2015-08-25 08:13:04 +0100 (Tue, 25 Aug 2015) $
...
#define DATE_STRING "$LastChangedDate: 2015-08-25 08:13:04 +0100 (Tue, 25 Aug 2015) $"
#define REV_STRING "$Rev: 271 $"
...
int detectobject(int, int, double **, int**, int*);
...
// this is used when marginalising over size and when selecting samples in the final ellipticity estimate
float Pcut = 0.01;

// set likelihood threshold at 0.05 in probability (i.e. enclosing 95% of a 2D gaussian)
// this is used when measure the likelihood surface
float Lcut = 0.01;
...
char **output_string, *temp_string;
...
/* get input environment variables */
	...
	fprintf(asciifile,"# 12  <PSF>-e1 \n");
	fprintf(asciifile,"# 13  <PSF>-e2 \n");
	fprintf(asciifile,"# 14  <PSF>-Strehl-ratio \n");
	fprintf(asciifile,"# 15  star-galaxy f-probability\n");
	fprintf(asciifile,"# 16  r correction \n"); //fit probability (not to be used for object selection)\n");
	fprintf(asciifile,"# 17  2D measurement variance \n");
	fprintf(asciifile,"# 18  mean-likelihood |e| \n");
	fprintf(asciifile,"# 19  e1 correction \n");
	fprintf(asciifile,"# 20  e2 correction \n");
	fprintf(asciifile,"# 21  neighbour mag \n");
	fprintf(asciifile,"# 22  neighbour distance \n");
	fprintf(asciifile,"# 23  catmag \n");
	fprintf(asciifile,"# 24  n-exposures-used \n");
	fprintf(asciifile,"# 25  cat-ID \n");
	...
	*/
	    ...
	    if (oversampling <= 1) {
	      ...
	      getospsfsizes(psfname, &correctd, &fwidth, &fheight, &psfoversampling,
			    &order[j], &chipvariation, &chiporder,
			    &ncoeffs, &nfit, numstars[j], &nxchip, &nychip,
			    &xchipsampling, &ychipsampling, &big_gap,
			    &hxsize[j], &hysize[j], &psfposerror[j], &scalefactor[j],
			    &sorder, &schipvariation, &schiporder,
			    &sncoeffs);
...
/* read the input catalogue */
    ...
    for (i=0; i<nobj; i++) output_string[i] = (char*) calloc(220, sizeof(char));
    temp_string = (char*)calloc(220, sizeof(char));
	...
	for (j=0; j<nobj; j++)
	  {
		...
		sep = 3600.*sqrt(
				      pow( ((wcsx[i]-wcsx[j])*cos(wcsy[i]*pi/180.)), 2 ) + 
				      pow( (wcsy[i]-wcsy[j]), 2 ) );
...
/* table to identify each object */
    ...
    for(i = 0; i < nobj; i++) {
		...
		strncpy(output_string[i]+38," 0.0000  1 ",10);
                tgoodfit--;
                tstarfit++;
                tswap++;
            }
...
detectobjloop(void *threadargs)
{
	...
	for(j = 0; j < nimages; j++) {
	  ...
	  dstatus = detectobject(nt, i, Dreal[i], pnobj[i], nnobj[i]);
	...
	iter = 0;
		...
		*/
		 ...
		 * and if it has enough exposures
			...
			measure_ellipticity(nt, sampling, maxlikel, &ecorr, &rmar[i], &gweight, &gvariance);
		...
		while (cabs(ecorr-oldecorr) > 0.0001 &&  // corrected ellipticity has changed by > .0001
		       ...
		       && fitclass[i] <= 0)              // excluding anything classified as star
		  {
		    ...
		    // use corrected galaxy parameters
		    // - find closest ready-made model
		    deltmin = 1.e10;
		    bestne = 0;
		    for (ne=0; ne<nume; ne++)
		...
		*/
		...
		// remeasure
		measure_ellipticity(nt, sampling, maxlikel, &emeasured, &rmeasured, &mweight, &mvariance);
	    ...
	    if (gvariance < 1./pow(profile_sn,2) ) 
	      {
		gweight = 0.;
		if (fitclass[i]==0)
		  {
	    ...
	    sprintf(output_string[i],
		    "%10.6lf %10.6lf %7.4f %7.4f %7.4f %2d %7.4f %6.4f %8.1f %8.1f %8.6f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %5.2f %5.2f %5.2f %3d %8d", 
		    wcsx[i], wcsy[i], creal(ecorr), cimag(ecorr), gweight,
		    fitclass[i], rmar[i], bulgefraction, totalflux, snratio[i], profile_sn,
		    meanpsfe[0], meanpsfe[1], meanstrehl, fprob[i], rdelta,
    ...
    )
{
    ...
    // test if contaninating catalogue objects are found in the target segmented region
    if (target >= 0) {
      for(j = 0; j < nimages; j++) {
...
void measure_ellipticity (int nt, int sampling, double maxlikel, 
		 ...
		 double *gweight, double *gvariance)
{
      ...
      if (wanted[nt][ne] == 1) 
	{
	  ...
	  if (marP[nt][ne] == NOT_MEASURED)
	      ...
	      if (rstep > 0)
		{
		  ...
		  new++;
		  if (currentobject[nt]==printlikel) 
		    {printf(" marginaliser %f %f marP %f \n",e1[ne],e2[ne],marP[nt][ne]);}
		}
	    }
